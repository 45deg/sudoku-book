...
今回の複数解問題では、メッセージは収束しましたが、8マスで最大の重みが同率となりました。
この実装には候補を分岐させる処理がないため、盤面を確定できず ``unknown`` です。
矛盾問題では、200反復しても収束せず ``unknown`` になりました。これらは今回の入力と設定での
結果です。未収束や同率の候補が残ることだけでは、解なしとは判定できません。
