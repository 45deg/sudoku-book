...
`連続時間SATソルバ <https://www.nature.com/articles/nphys2105>`_ では、変数を
:math:`s_i\in[-1,1]` の連続値とし、端点の :math:`s_i=1` を真、:math:`s_i=-1` を偽に対応させます。
各SAT節 :math:`m` の違反量を関数 :math:`K_m` で測ります。
...
:math:`1-c_{mi}s_i=0` です。積全体も0になり、節が満たされたことを表します。
すべての変数が端点の±1にある場合は、節の全リテラルが偽のときだけ違反量が正になります。
途中の連続値では、各変数はまだ真偽に確定していません。たとえば二つの肯定リテラルからなる節で
両変数が0なら、:math:`K_m=1/4` です。このように、真偽に確定する前の値にも違反量を定義できます。
